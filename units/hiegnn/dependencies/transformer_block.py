...
class SO2EquivariantGraphAttention(torch.nn.Module):
    ...
    def forward(self, x, atomic_numbers, edge_distance, edge_index):
        ...
        x_message._reduce_edge(edge_index[1], x.embedding.shape[0])
...
class FeedForwardNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        self.max_lmax: int = max(self.lmax_list)
...
class TransBlockV2(torch.nn.Module):
    ...
    def forward(self, x, atomic_numbers, edge_distance, edge_index, batch):  # SO3_Embedding  # for GraphDropPath
